...
for dist in TESTING_VALUES:  # radius we are interested in.
    ...
    for XCorr in XC_epochs:
        ...
        x_mesh_raveled = x_mesh.ravel()
        y_mesh_raveled = y_mesh.ravel()
        fitting_gauss = LevMarLSQFitter()  # Fitting method; Levenberg-Marquardt Least Squares algorithm
        best_fit_gauss = fitting_gauss(gauss_init, x_mesh_raveled, y_mesh_raveled, XCorr.ravel())  # The best fit for the map
...
# form = '%s'
# np.savetxt('/home/broughtonco/documents/nrc/data/SERPENS_MAIN/Datafiles/EXTENDED_SERPENS_MAIN_TOPCAT.table',
#            li[1:],
#            fmt=form,
#            header=hdr
#            )
