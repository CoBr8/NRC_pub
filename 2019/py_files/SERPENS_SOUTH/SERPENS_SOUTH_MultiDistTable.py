...
root = '/home/broughtonco/documents/nrc/data/SERPENS_SOUTH/'  # the root of the data folder
files = os.listdir(root)  # listing all the files in root.

FirstEpoch = fits.open('/home/broughtonco/documents/nrc/data/SERPENS_SOUTH/SERPENS_SOUTH_20160202_00058_850_EA3_cal.fit')
...
for date, (hdu, Vec) in sorted(dictionary1.items(), key=op.itemgetter(0)):  # pulling data from dictionary
    ...
    for data, name in zip(dat, Dat_Titles):  # updating the fits files every time, remove old ones first.
        if os.path.exists('/home/broughtonco/documents/nrc/data/SERPENS_SOUTH/GeneratedMapsFits/{}_{}_{}.fit'.format(
                Region_Name, date, name)):
            os.remove('/home/broughtonco/documents/nrc/data/SERPENS_SOUTH/GeneratedMapsFits/{}_{}_{}.fit'.format(
                Region_Name, date, name))
        hdu = fits.PrimaryHDU(data)
        hdu.writeto(
            '/home/broughtonco/documents/nrc/data/SERPENS_SOUTH/GeneratedMapsFits/{}_{}_{}.fit'.format(Region_Name, date, name))
...
MetaData = np.loadtxt('/home/broughtonco/documents/nrc/data/SERPENS_SOUTH/SERPENS_SOUTH_850_EA3_cal_metadata.txt', dtype='str')
...
form = '%s'
np.savetxt('/home/broughtonco/documents/nrc/data/SERPENS_SOUTH/Datafiles/EXTENDED_SERPENS_SOUTH_TOPCAT.table',
           li[1:],
           fmt=form,
           header=hdr
           )
