...
# +========================================+
i = 0
for AC, B in zip(AC_epochs, Dat_dict[5][0][:][1]):
    # figuring out where i need to clip to
    ...
    gauss_init = Gaussian2D(
        amplitude=AC.max(),
        x_mean=np.where(AC == AC.max())[1],  # location to start fitting gaussian
        y_mean=np.where(AC == AC.max())[0],  # location to start fitting gaussian
    ...
    AC_fit_dict[i] = ((best_fit_gauss.amplitude.value, err[0]),
                      (best_fit_gauss.x_stddev.value, err[3]),
                      (best_fit_gauss.y_stddev.value, err[4])
                      )
    i += 1
...
# AC_fit.set_title('AC Map')
# AC_fit.scatter(r[1:num], AC_Data[Epoch - 1][1:num].real, s=1, marker=',', lw=0, color='red')
# AC_fit.plot(r[1:num], f(r[1:num], fit_ac[Epoch - 1][0], fit_ac[Epoch - 1][1]),
#             linestyle='--',
#             color='green',
#             linewidth=0.5)
...
# AC_DIV.scatter(r[1:num], DivACData[Epoch - 1][1:num].real, s=1, marker=',', lw=0, color='red')
# AC_DIV.plot(r[1:num], f(r[1:num], fit_ac_div[Epoch - 1][0], fit_ac_div[Epoch - 1][1]),
#             linestyle='--',
#             color='green',
#             linewidth=0.5)
...
hdr = 'Epoch Name JD Elevation T225 RMS Steve_offset_x Steve_offset_y Cal_f Cal_f_err ' \
      ...
      'BD BD_err MD MD_err AC_amp AC_amp_err AC_sig_x AC_sig_x_err AC_sig_y AC_sig_y_err'
...
for epoch in range(len(AC_Data)):
    ...
    AC_amp = AC_fit_dict[epoch][0][0]  # amplitudes
    AC_amp_err = AC_fit_dict[epoch][0][1]  # amplitude error
    AC_sig_x = AC_fit_dict[epoch][1][0]
    AC_sig_x_err = AC_fit_dict[epoch][1][1]
    AC_sig_y = AC_fit_dict[epoch][2][0]
    AC_sig_y_err = AC_fit_dict[epoch][2][0]
