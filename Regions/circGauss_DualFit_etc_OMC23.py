...
# TODO: Fix the gaussian to only use sigma_x = sigma_y
...
for XCorr in XC_epochs:
    ...
    XCorr = XCorr[Y_max - 15:Y_max + 16, X_max - 15:X_max + 16]
...
hdr = 'Epoch Name JD Elevation T225 RMS Steve_offset_x Steve_offset_y Cal_f Cal_f_err ' \
      'BA BA_err MA MA_err BAP BAP_err MAP MAP_err Covariance_div ' \
...
for i in range(len(AC_Data)):
    ...
    opt_fit_AC_2, cov_mat_AC_2 = curve_fit(f2, r[1:num], AC_Data[i][1:num])
    ...
    fit_ac_2.append(opt_fit_AC_2)
    cov_ac_2.append(cov_mat_AC_2)
    err_ac_2.append(np.sqrt(np.diag(cov_mat_AC_2)))
...
Dat_dict_2[dist] = [fit_ac_2, err_ac_2]
...
for epoch in range(len(AC_Data)):
    ...
    BA_parabola = Dat_dict[5][0][epoch][1]
    BA_parabola_err = Dat_dict[5][1][epoch][1]
    MA_parabola = Dat_dict[5][0][epoch][0]
    MA_parabola_err = Dat_dict[5][1][epoch][0]
    ...
    dat = np.array([e_num, name, jd, elev, t225, rms, *steve_offset, cal_f, cal_f_err,
                    BA, BA_err, MA, MA_err,
                    BA_parabola, BA_parabola_err, MA_parabola, MA_parabola_err,
                    covariance, *jcmt_offset,
...
np.savetxt('/home/broughtonco/documents/nrc/data/OMC23/Datafiles/parabola_OMC23_TABLE_TOPCAT.table',
           li[1:],
           fmt=form,
           header=hdr
           )