...
root = '/home/cobr/Documents/NRC/data/IC348/850/'  # the root of the data folder
files = os.listdir(root)  # listing all the files in root.

FirstEpoch = fits.open('/home/cobr/Documents/NRC/data/IC348/850/IC348_20151222_00019_850_EA3_cal.fit')
    ...
    for XCorr in XC_epochs:
        ...
        gauss_init = Gaussian2D(
            ...
            bounds={
                  ...
                  'x_mean': (int(np.where(XCorr == XCorr.max())[1]) - 2, int(np.where(XCorr == XCorr.max())[1]) + 2),
                  'y_mean': (int(np.where(XCorr == XCorr.max())[0]) - 2, int(np.where(XCorr == XCorr.max())[0]) + 2)
...
MetaData = np.loadtxt('/home/cobr/Documents/NRC/data/IC348/850/IC348_850_EA3_cal_metadata.txt', dtype='str')
...
# =================
form = '%s'
np.savetxt('/home/cobr/Documents/NRC/data/IC348/850/Datafiles/MASKED_IC348.table',
           li[1:],
           fmt=form,
           header=hdr
           )
