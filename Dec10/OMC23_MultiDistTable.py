...
for dist in TESTING_VALUES:  # radius we are interested in.
    ...
    for i, AC in enumerate(AC_epochs):
        ...
        x_mesh = np.ma.array(x_mesh, mask=False)
        y_mesh = np.ma.array(y_mesh, mask=False)

        x_mesh.mask[x_mesh.shape[0] // 2, x_mesh.shape[1] // 2] = True
        y_mesh.mask[y_mesh.shape[0] // 2, y_mesh.shape[0] // 2] = True
...
form = '%s'
np.savetxt('/home/broughtonco/documents/nrc/data/OMC23/Datafiles/omc23-extended-maskedAC.table',
           li[1:],
           fmt=form,
           header=hdr
           )
